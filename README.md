...
For Single-Stage: Theoretically single stage setup for classes should work the same but it doesn't which seems like a bug in mmdetection. So metainfo from `mmdetection/custom_configs/detr.py` will not be copied over so you have to manually copy the classes to `cuneiform-ocr/mmdetection/mmdet/datasets/coco.py`. The classes are determined in the train_instances.json and test_instances.json (at the bottom) from the data folder (data-recognition).
