...
def js(A, B,pred_idx, src_idx, thresh):
    if abs(pred_idx - src_idx) > thresh:
        return 0
    cummulative_score = 0
    longest_list = B
    shortest_list = A
...
def line_alignment(pred, src, thresh=3):
    line_alignment_result = defaultdict(list)
    for idx, pred_line in enumerate(pred):
        lines_scores = np.zeros(len(src))
        for src_idx, src_line in enumerate(src):
            lines_scores[src_idx] = js(pred_line.split(' '), src_line.split(' '), idx, src_idx, thresh)
        order = lines_scores.argmax()
        line_alignment_result[pred_line].append(src[order])
    return line_alignment_result
...
if __name__ == "__main__":
    ...
    result = line_alignment(pred, src)
    print(result)