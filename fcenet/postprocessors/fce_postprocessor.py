...
from ..utils import TextDetDataSample, fill_hole, polys2shapely
...
class FCEPostprocessor(BaseTextDetPostProcessor):
    ...
    def _process_single(self, pred_result: Union[Tensor, List[Tensor]],
        ...
        # custom override by Yunus Cobanoglu not part of mmocr
        # make it compatible with mmdet
        result = super()._process_single(pred_result, data_sample, **kwargs)

        polys = result.pred_instances["polygons"]
        result.pred_instances["bboxes"] = torch.from_numpy(np.asarray([poly.bounds for poly in polys2shapely(polys)]))
        result.pred_instances["labels"] = torch.from_numpy(np.array([0 for _ in range(len(polys))]))
        result.gt_instances.bboxes = HorizontalBoxes.cxcywh_to_xyxy(result.gt_instances.bboxes.cxcywh)
        return result
